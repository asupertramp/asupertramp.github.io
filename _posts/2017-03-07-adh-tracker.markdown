...
从而可得，t-1时刻的观测值只与t-1时刻的St-1有关。

<img src="/images/posts/2017-03-07/fig6.png">

关于P(St|St-1),代表在已知t-1时刻的采样点集时，t时刻采样点集的概率。由于随机采样，遮挡，视角等变化，t时刻的采样点会变化。t时刻的采样点要不来自于上一时刻的可见部分，要不来自于上一时刻被遮挡的部分。令P(V)代表从可见部分采样的概率，则有：

<img src="/images/posts/2017-03-07/fig7.png">

P(St,j|St-1, V)可以建模为高斯分布，方差由传感器的分辨率导出。
对于P(St,j|St-1, -V)我们可以上一时刻不可见的部分都是被遮挡的部分，因此有

<img src="/images/posts/2017-03-07/fig8.png">

两者存在线性关系，但两者的和并不为1.
最终可以写成：

```
P(St,j|St-1)=η(p(st,j |st−1,V)+k)
    η = p(V ) − p(¬V )k1
    k = p(¬V )k1k2/η
```
